...
void Asset::genPreviewGlBuffersPerNode(NodeItem& node) {
    ...
    if (np.type == T::TRIANGLE) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3) * 3, &t.v[0],
                     GL_DYNAMIC_DRAW);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, node.glVBO);
        glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3) * bf.size(), &bf[0],
                     GL_DYNAMIC_DRAW);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                     sizeof(unsigned int) * m.positionIndices.size(),
                     &m.positionIndices[0], GL_DYNAMIC_DRAW);
...
void Asset::genPreviewGlBuffersPerLight(LightItem& light) {
    ...
    if (lp.type == T::POINT) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3) * ps.size(), &ps[0],
                     GL_STATIC_DRAW);
