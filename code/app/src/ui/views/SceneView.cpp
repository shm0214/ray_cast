...
namespace NRenderer {
SceneView::SceneView(const Vec2& position,
                     const Vec2& size,
                     UIContext& uiContext,
                     Manager& manager)
...
void SceneView::cameraSetting() {
    ...
    ImGui::InputScalar("Fov", ImGuiDataType_Float, &camera.fov, &floatStep,
                       NULL);
    ImGui::InputScalar("Aspect", ImGuiDataType_Float, &camera.aspect,
                       &floatStep, NULL);
    ImGui::InputScalar("Aperture", ImGuiDataType_Float, &camera.aperture,
                       &floatStep, NULL);
...
void SceneView::renderSetting() {
    int intStep = 1;
    float floatStep = 0.01;
    auto& rs = manager.renderSettingsManager.renderSettings;
    ImGui::TextUnformatted("Render Settings:");
    ImGui::InputScalar("Width", ImGuiDataType_U32, &rs.width, &intStep, NULL,
                       "%u");
    ImGui::InputScalar("Height", ImGuiDataType_U32, &rs.height, &intStep, NULL,
                       "%u");
    ImGui::InputScalar("Depth", ImGuiDataType_U32, &rs.depth, &intStep, NULL,
                       "%u");
...
void SceneView::componentSetting() {
    ...
    auto&& components =
        getServer().componentFactory.getComponentsInfo("Render");
    ...
    if (currComponentSelected != -1 &&
        ...
        ImGui::TextWrapped(
            components[currComponentSelected].description.c_str());
    }
    if (ImGui::Button("Render")) {
        if (currComponentSelected != -1 &&
            currComponentSelected < components.size()) {
            auto& rs = manager.renderSettingsManager;
            SceneBuilder sceneBuilder{manager.assetManager.asset,
                                      rs.renderSettings, rs.ambientSettings,
                                      rs.camera};
